/* wallet_wc_v2.js – WalletConnect V2 + MetaMask/Exodus Extension
   החלף את YOUR_WALLETCONNECT_V2_PROJECT_ID בפרויקט שלך מה־WalletConnect Cloud.
*/
...
async function connectWCv2(){
  ...
  const wc = await WCEProvider.init({
    ...
    metadata: { name:'Crypto Hidden Object Game', description:'Game with ETH wallet', url:location.origin, icons:[] }
  });
  wc.on('display_uri',(uri)=>{
    if(isMobile()) location.href = `exodus://wc?uri=${encodeURIComponent(uri)}`;
  });
  await wc.connect();
