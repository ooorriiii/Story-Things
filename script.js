להדבקה בקובץ script.js. הסקריפט מניח שקיימים האלמנטים שהוגדרו ב‑index\_final.html ושהמודול wallet\_exodus\_integration.js נטען.
...
function playTone(freq, duration){
  ...
  gain.gain.setValueAtTime(0.1, t); gain.gain.exponentialRampToValueAtTime(0.001, t+duration);
  osc.stop(t+duration);
}
const SFX={collect:()=>playTone(850,.15),bonus:()=>playTone(1000,.2),time:()=>playTone(600,.25),life:()=>playTone(400,.25),bomb:()=>playTone(150,.3),boss:()=>playTone(350,.5),purchase:()=>playTone(500,.2),achv:()=>playTone(700,.4)}
