// Extended game script with sound effects. Based on script_ultimate.js.

// Audio context and helper functions for sounds
const AudioContextClass = window.AudioContext || window.webkitAudioContext;
const audioCtx = new AudioContextClass();

// Play a single tone of given frequency and duration (in seconds)
...
// The rest of the code mirrors script_ultimate.js but with calls to play sounds at key moments.
...
function startBombMovement() {
  ...
  bombInterval = setInterval(() => {
    ...
    bombs.forEach(bombObj => {
      const { element } = bombObj;
      let x = parseFloat(element.style.left);
      let y = parseFloat(element.style.top);
      const speedFactor = slowBombActive ? 0.5 : 1;
      let newX = x + bombObj.dx * speedFactor;
      let newY = y + bombObj.dy * speedFactor;
      if (newX < 0 || newX > 95) {
        bombObj.dx *= -1;
